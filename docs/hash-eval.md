...
- **Akita (offload):** the same pin and field as the direct row above, planned
  against the `fp<width>-dense-recursive` catalog so setup work is offloaded
  onto a setup-prefix edge. Upstream ships no dense recursive family, so all
  three are generated by `scripts/extend-akita-dense-offload.sh <checkout>
  <field>` and installed from `vendor/akita-catalogs/`. A row is offloadable
