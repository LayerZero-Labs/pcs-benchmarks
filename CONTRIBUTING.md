...
For lattice-table or hash-table changes, include the exact command used, explain any workload or
security-assumption change, and keep `results/lattice-x86_64/` /
`results/hash-x86_64/` in sync with the JSONL that produced the report. Never update a PCS revision in the same commit
